...
<h3 class="text-primary"><!--como fizemos em consultar-->
    Cadastrar alunos (planilha SIGA)<br><!-- comment -->
    Os campos E-mail e Fone podem ser inseridos nessa planilha
</h3>
    ...
    if ($xlsx = $xls->parse($_FILES['arquivo']['tmp_name'])) {
        ...
        foreach ($xlsx->rows() as $mostrar) {
            if ($i > 2) {
                ...
                $al->setRa(trim($mostrar[0]));
                $al->setNome(trim($mostrar[1]));
                $al->setTipo_trabalho($tipotrab);
                
                isset($mostrar[3]) ? $al->setEmail($mostrar[3]) : "";
                isset($mostrar[4]) ? $al->setFone($mostrar[4]) : "";                
