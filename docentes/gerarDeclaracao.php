...
<div class="card shadow"><!-- acrescentei um card com sombra aqui tbm -->
    <form method="post" name="formsalvar" id="formSalvar" class="m-3">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-4">            
                <input type="submit" 
                       class="btn btn-primary" name="btngerartg" value="Declarações TG">               
                <input type="submit" 
                       class="btn btn-success" name="btngerarptg" value="Declarações PTG">               
            </div>
        </div>
    </form>
</div>
...
<h4 style="color: red; text-align: center;" class="col-sm-12 col-md-12">
    ...
    if (filter_input(INPUT_POST, 'btngerartg')) {
        ...
        ?>
        <a style="color: red; " class="nav-link" onclick="printPage('?p=bancatg/gerarDeclaracoes&semestre=<?= $ano . $semestre ?>');" href="#">
            Clique aqui para imprimir Declarações TG de <?= $ano ?> / <?= $semestre ?>º semestre
        </a>
        <?php
    }
    if (filter_input(INPUT_POST, 'btngerarptg')) {
        ...
        $semestre = filter_input(INPUT_POST, 'selsemestre', FILTER_SANITIZE_NUMBER_INT);
        ?>
        <a style="color: red;" class="nav-link" onclick="printPage('?p=bancaptg/gerarDeclaracoes&semestre=<?= $ano . $semestre ?>');" href="#">
            Clique aqui para imprimir Declarações PTG  de <?= $ano ?> / <?= $semestre ?>º semestre
        </a>
        <?php
    }
    ?>
